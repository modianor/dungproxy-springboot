...
public class DistributeService {
    ...
    private List<DomainIp> get4DomainTested(RequestForm requestForm) {
        ...
        // 根据domain去domainIp库里查找可用IP
        List<DomainIp> domainIps = domainIpRepository.selectAvailable(domain, PageRequest.of(0, Integer.MAX_VALUE));
        // 如果domainIp没有可用IP
        if (StringUtils.isNotEmpty(checkUrl)) {
            // 当前url正在使用,属于活跃域名,所以发送到检查模块,没有毛病
